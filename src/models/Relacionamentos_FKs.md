...
erDiagram
  ...
  users ||--o{ platform_integrations : integracoes_admin
```
...
|---------------|--------|-------|---------------------|
| `platform_integrations` | `configured_by_user_id` | N:1 opcional | `users.id` *(gestora/atendimento com permissão ao painel técnico)* |
...
| `platform_integrations` | (`provider`,`instance_name`) | Uma única linha ativa por par provedor-instância (evita clones acidentais no Evolution/Chatwoot) |
...
### Integrações administrativas (`platform_integrations`)

- Credenciais cifradas, QR Evolution e IDs Chatwoot expostos somente pela API admin autenticada (proxy Gestora vs provedores externos).
- `configured_by_user_id` liga revisões rápidas a um `users.id` Gestora/atendimento privilegiado.
- Índice parcial garante apenas **uma** configuração ativa por tupla `(provider, instance_name)` para impedir clones acidentais.
